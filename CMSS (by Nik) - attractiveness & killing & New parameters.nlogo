...
;-----------------------------------------------------------------------------------------------
; structs (object-specific values, here: agent-specific)
...
[
 ...
 ; Info: Add Sliders for Attractiveness Scales:
 ; in Interface, select "Slider" button. Name = male_attractiveness_scale, min = 1, max = 10, initial = chose 1-10.
 ; Same for female_attractiveness_scale. Save and click on the checkmark. Then the error "can't find" should disappear.

 ; Info: Add a Monitor: in the interface, select "Monitor" (Button). Reporter = numberRejections, label (optional) = Number of Rejections.
]

;-----------------------------------------------------------------------------------------------
to setup       ; this function (and the button with the same name) resets all the values to the standard values

  clear-all ; delete, now reset to set standard values:
  ...
  [
    ...
    set isFertile true
    set attractiveness random 1 + male_attractiveness_scale  ; <----------- NEW (3)
...
;-----------------------------------------------------------------------------------------------
to go        ; this function (and the button with the same name) runs and ends the simulation
...
;-----------------------------------------------------------------------------------------------
to actMales     ; this function enables the male agents to move forward. They apparently only move

  ask males
  [
    forward random 6
    right random 90 ; after moving, they rotate in a 90 degree angle to avoid getting stuck
  ]
end

;-----------------------------------------------------------------------------------------------
to actFemals     ; this function enables the female agents to act
  ...
  [
    ...
    ; from here on, this is all specific female-behaviour:
  ...
  repeat births
  [
    let isTwin random 100 > 97  ; <----------- NEW (recent data)
  ...
  ] ; end of repeat births

  ask turtles with [ ageDays = 1 ]
  [
    setxy 0 0 ; set x/y coordinates
  ]
end

;-----------------------------------------------------------------------------------------------
to checkAccident ; this function handles accidents
...
;-----------------------------------------------------------------------------------------------
to checkLife                 ; this function handles life exptectancy
...
;-----------------------------------------------------------------------------------------------
to countSex    ; this function handles population count
...
;-----------------------------------------------------------------------------------------------
to determineMeanAges     ; this function handles aging
...
;-----------------------------------------------------------------------------------------------
to determineYear ; this function handles the passing of time

  set currentYear ticks / 365
end

;-----------------------------------------------------------------------------------------------
to mate           ; this function handles reproduction
  ...
  [
    ...
    [
      let attractivenessMatch (attractiveness - [attractiveness] of partner)    ; <----------- NEW (3)
      let acceptProbability (1 - abs(attractivenessMatch) / 10)                     ; <----------- NEW (3)
      if random-float 1 <= acceptProbability and random-float 1 <= acceptProbability    ; <----------- NEW (3)
      [
        let threshold_new_pregnant 20 + random 51 ; <----------- NEW (new data)
        let getsPregnant random 100 > threshold_new_pregnant ; <----------- NEW (new data)
        ;let getsPregnant random 100 > 75 ; <----------- NEW (old data & code)
        if getsPregnant
        [
          ask partner
          [
      ...
      if not (random-float 1 <= acceptProbability and random-float 1 <= acceptProbability)    ; <----------- NEW (3)
      [
        set numberRejections numberRejections + 1    ; <----------- NEW (3)
      ]
    ]
    if not (partner != nobody and isFertile and [isFertile] of partner and [ previousChildren < maxChildren ] of partner)    ; <----------- NEW (3)
    [
      set numberRejections numberRejections + 1    ; <----------- NEW (3)
    ]
  ]
...
;-----------------------------------------------------------------------------------------------
to-report missionEnds?           ; this function handles end-of-journey conditions
  if (currentYear > 6300)
  [
    clear-output
...
;-----------------------------------------------------------------------------------------------
to moveAwayFromSpace    ; this function handles scenarios of agents leaving the zone
...
;-----------------------------------------------------------------------------------------------
to protect     ; this function handles the safe-zone
...
;-----------------------------------------------------------------------------------------------
to updatePlots    ; this function handles graphical visualisation
...
;----------------------------------------------------------------------------------------------- <------ NEW (*)
to checkPolarizingView ; this is the killing logic <------ NEW (*)

  ask turtles with [pcolor > 10] ; only agents outside of the saftey-zone (= pcolor < 9.9)
  [
...
<experiments>
  <experiment name="test experiment" repetitions="1" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="maxAgeFemals">
      <value value="73.8"/>  ; <----------- NEW (recent data)
    </enumeratedValueSet>
